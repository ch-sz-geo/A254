// Erklärung und Link auf Website
ifdef::env-github[]
[discrete]
= Grundwasseraustritte und -fassungen (A254)
Sie befinden sich auf der Startseite des Themas **"Grundwasseraustritte und -fassungen (A254)"**. Über folgende Links gelangen Sie zu weiteren Informationen:

* https://ch-sz-geo.github.io/A254/[Daten, Datenmodell, Modellbeschreibung, Erfassungsrichtlinie usw.] zu diesem Thema
* https://ch-sz-geo.github.io/A000_entrypage/[Einstiegsseite] mit weiteren Themenbeschreibungen
* https://data.geo.sz.ch/public/Themen/Themenliste.xml[Übersicht der Themen] mit der Zusammenstellung weiterer Beschreibungen
endif::[]
